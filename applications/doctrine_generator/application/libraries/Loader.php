...
// {{{ Matchbox
    ...
    function _mb_load($resource, $modules, $basepath = FALSE)
    {
        ...
        else
        {
            ...
            {
                $paths[] = APPPATH;
                $paths[] = realpath( FCPATH . implode(DIRECTORY_SEPARATOR, array('..','..','common')) ) . DIRECTORY_SEPARATOR;
            }
    ...
    // }}}
    // [[[ Arquitectura
    function module_xml($module, $xml, $app = FALSE)
    {
        if($module == '')
            return FALSE;
        return $this->xml($xml, $app, $module);
    }

    function xml($xml, $app = FALSE, $module = '')
    {
        $paths = array();
        if($module != '')
        {
        ...
        if($app)
        {
            $paths[] = realpath(APPPATH.'../');
        }
        $paths[] = realpath( FCPATH . implode(DIRECTORY_SEPARATOR, array('..','..','common')) ) . DIRECTORY_SEPARATOR;
        
        foreach ($paths as $path)
        {
            $filepath = $path.'xml'.DIRECTORY_SEPARATOR.str_replace('.xml', '', $xml).'.xml';
